...
describe("tables", () => {
  it("parses tables", () => {
    for (const tableName of getTableNames()) {
      ...
      expect(recalculatedTotalWeight).toEqual(totalWeight);
    }
  });
  ...
  it("has valid table schema", () => {
    ...
    function processSchema(schemaElement: any, paths: string[], defs: DefsMap) {
      if (typeof schemaElement === "string") {
        ...
        expect(typeof schemaElement).toBe("object");
    ...
    function processGroups(groupsAnalysis: StaticAnalysis[], tablePaths: string[], prevDef: DefsMap = {}): Row {
      ...
      function checkIsDefined(node: AnalysisNode) {
        ...
        expect(groupsDefinitions).toHaveProperty(node.name);
        expect(groupsDefinitions[node.name].type).toEqual(node.type);
      }
  ...
  is defining "${def.name}" with type "${def.type}", but it has type "${a[key].def.type}" in other rows`,
                ...
                expect(def.type).toEqual(a[key].def.type);
              }
              return a;
            }, p),
          {} as {
      ...
      for (const item of groupsAnalysis) {
        ...
        if (item.table) {
          ...
          if (isRecursive) {
            ...
            expect(tableAnalysis.analysis.length).toBeGreaterThan(1);
            hasRecursion = true;
